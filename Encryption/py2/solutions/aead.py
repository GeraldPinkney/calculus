...
def encrypt_record(record, key, nonce):
    ...
    plaintext = '{0}\x1f{1}\x1f{2}\x1f{3}\x1f{4}'.format(
        record['Address'], record['Nationality'], record['DOB'], record['SSN'], record['Phone']
    )
    print (plaintext)
    # Perform the same process for the authenticated fields, those that aren't encrypted
    ad = '{0}\x1f{1}\x1f{2}\x1f{3}\x1f{4}'.format(
        record['ID'], record['Surname'], record['FirstName'], record['JoinDate'], record['LastLogin']
    )
...
def decrypt_record(record, key):
    ...
    aesgcm = AESGCM(key)
    plaintext = aesgcm.decrypt(record["Nonce"], record["Data"], ad).split('\x1f')
...
for k,v in record.items():
    if v:
        print (" ", k, ":", v.encode("string_escape"))
    else:
        print(" ", k, ":", v)
print("}")
